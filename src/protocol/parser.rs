use crate::protocol::{
    Action, SpopFrame, VarScope,
    frame::{FrameFlags, FramePayload, FrameType, Message, Metadata},
    frames::{
        Ack, AgentDisconnect, AgentDisconnectFrame, AgentHello,
        agent_hello::AgentHelloFrame,
...
pub fn parse_frame(input: &[u8]) -> IResult<&[u8], Box<dyn SpopFrame>> {
    ...
    match frame_type {
        ...
        FrameType::AgentHello => {
            ...
            let hello = AgentHello::try_from(payload)
                .map_err(|_| nom::Err::Error(Error::new(input, ErrorKind::Tag)))?;

            let frame = AgentHelloFrame {
                metadata,
                payload: hello,
            };
        ...
        FrameType::AgentDisconnect => {
            ...
            // check mandatory items
            let disconnect = AgentDisconnect::try_from(payload)
                .map_err(|_| nom::Err::Error(Error::new(input, ErrorKind::Tag)))?;

            let frame = AgentDisconnectFrame {
                metadata,
                payload: disconnect,
            };
        ...
        FrameType::Ack => {
            let mut parser = all_consuming(parse_list_of_actions);

            let (_, actions) = parser.parse(frame_payload)?;
...
fn parse_action(input: &[u8]) -> IResult<&[u8], Action> {
    let (remaining, action_type) = be_u8(input)?;
    let (remaining, nb_args) = be_u8(remaining)?;
    let (remaining, scope) = be_u8(remaining)?;
    ...
    if action_type == 1 {
        ...
        let scope = VarScope::try_from(scope)
            .map_err(|_| nom::Err::Error(Error::new(input, ErrorKind::Tag)))?;

        let (remaining, name) = parse_string(remaining)?;
        let (remaining, value) = typed_data(remaining)?;

        Ok((remaining, Action::SetVar { scope, name, value }))
        ...
        Ok((remaining, Action::UnSetVar { scope, name }))
    } else {
        Err(nom::Err::Failure(Error::new(input, ErrorKind::Tag)))
    }
}

fn parse_list_of_actions(input: &[u8]) -> IResult<&[u8], Vec<Action>> {
    let mut parser = all_consuming(many0(complete(parse_action)));
    parser.parse(input)
}
